...
class Localization_Tracker():
    ...
    def crop_tracklets(self,pre_locations,frame):
        ...
        # convert a priori locations into normalized crop coordinates to feed to localizer
        boxes = torch.from_numpy(boxes)
        prevs = torch.zeros(boxes.shape)
        prevs[:,0] = boxes[:,0] - boxes[:,2]/2.0
        prevs[:,2] = boxes[:,0] + boxes[:,2]/2.0
        prevs[:,1] = boxes[:,1] - boxes[:,2]*boxes[:,3]/2.0
        prevs[:,3] = boxes[:,1] + boxes[:,2]*boxes[:,3]/2.0
        ...
        return crops,new_boxes,box_ids,box_scales,scaled_prevs
    ...
    def remove_anomalies(self,max_size = 400):
        """
    ...
    def track(self):
        ...
        while frame_num != -1:            
            ...
            # skip  if there are no active tracklets or no localizer (KF prediction with no update)    
                ...
                crops,new_boxes,box_ids,box_scales,prevs = self.crop_tracklets(pre_locations,frame)
                
                
                # localize objects using localizer
                start= time.time()
                with torch.no_grad():                       
                    cls_out,reg_out = self.localizer(crops,prevs)
                
                # unsupress to verify output is correct
                if  False:
                    self.test_outputs(reg_out,crops)
                    time.sleep(2)
                    plt.close()
