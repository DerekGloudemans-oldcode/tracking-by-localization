...
from _localizers.pytorch_retinanet_localizer.retinanet.model_orig import resnet34
    ...
    for ber in bers:
        ...
        for iteration in range(n_iterations):
            ...
            with torch.no_grad():
                ...
                reg_boxes,confs,classes = localizer(crops,LOCALIZE = True)
                ...
                center_score = (20-x_diff-y_diff)/ 20.0
                width_score =  (20-w_diff-h_diff)/ 20.0
                
                score = alpha*confs + beta*center_score + gamma*width_score + delta*iou_score
                best_scores ,keep = torch.max(score,dim = 1)
                idx = torch.arange(reg_boxes.shape[0])
                detections = reg_boxes[idx,keep,:]
                score = score[idx,keep]
                confs = confs[idx,keep]
                classes = classes[idx,keep]
        
                #detections = a_priori[:,0,:]
...
if __name__ == "__main__":
    ...
    QFIT = os.path.join(filter_directory,"detrac_7_Q_ret.cpkl")
    QRFIT = os.path.join(filter_directory,"detrac_7_QR_ret.cpkl")
    QRRFIT = os.path.join(filter_directory, "detrac_7_QRR_ret.cpkl")
    ...
    localizer = resnet34(num_classes = 13,pretrained = True)
    
    loc_cp = "/home/worklab/Documents/code/tracking-by-localization/_localizers/pytorch_retinanet_localizer/cpu_ret_epoch6.pt"
    ...
    localizer.load_state_dict(new_state_dict)
